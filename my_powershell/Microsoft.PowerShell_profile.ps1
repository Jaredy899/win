...
if ($sessionIsInteractive) {
    ...
    try {
        # Simple internet check (you can replace 'github.com' with another reliable domain or IP)
        $response = Invoke-WebRequest -Uri "https://www.google.com" -UseBasicParsing -TimeoutSec 1
...
# Run the rest of the script only if interactive
if ($sessionIsInteractive) {
